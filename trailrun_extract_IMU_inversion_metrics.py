...
GPStiming = pd.read_csv('C:\\Users\\max.ferguson\\OneDrive - BOA Technology Inc\\Documents\\TrailAnkleMetrics\\Trail\\CombinedGPS.csv')
# Obtain IMU signals
fPath = 'C:\\Users\\max.ferguson\\OneDrive - BOA Technology Inc\\Documents\\TrailAnkleMetrics\\Trail\\'
...
for ii in range(0,len(GPStiming)):
    ...
    if GoodR == 1:
        ...
        for jj in RGS:
            ...
            pEgyro.append(np.abs(np.min(Rgyr[RHS[jj]:appTO,2])))
            pIgyro.append(-np.max(Rgyr[RHS[jj]:appTO,2]))   # flip so inversion reads negative
    ...
    if GoodL == 1:
        ...
        for jj in LGS:
            ...
            pEgyro.append(np.max(Lgyr[LHS[jj]:appTO,2]))
            pIgyro.append(np.min(Lgyr[LHS[jj]:appTO,2]))     # already negative
...
outcomes = pd.DataFrame({'Subject':list(oSubject), 'Side':list(oSide), 'Config': list(oConfig),'Sesh': list(oSesh),
                          'Label':list(oLabel), 'pJerk':list(pJerk), 'pAcc':list(pAcc), 'pGyr':list(pGyr),'rMLacc':list(rMLacc),'rIEgyro':list(rIEgyro),'pEgyro':list(pEgyro), 'pIgyro':list(pIgyro)})#,'imuSpeed':list(oSpeed)})

if save_on == 1:
    outcomes.to_csv('C:\\Users\\max.ferguson\\OneDrive - BOA Technology Inc\\Documents\\TrailAnkleMetrics\\Trail\\IMUmetrics_app.csv',header=True)
elif save_on == 2:
    outcomes.to_csv('C:\\Users\\max.ferguson\\OneDrive - BOA Technology Inc\\Documents\\TrailAnkleMetrics\\Trail\\IMUmetrics.csv',mode = 'a',header=False)
...
ax.set_title(f'pIgyro across all strides (n={len(p):,}) at {gyr_cut} Hz Cutoff')
...
summary['Mean'] = np.average(pIgyro)
summary['SD'] = np.std(pIgyro)
print(summary)

summary.to_csv('C:\\Users\\max.ferguson\\OneDrive - BOA Technology Inc\\Documents\\TrailAnkleMetrics\\Trail\\condition_summary.csv', index=True)
# plt.figure(3)
# plt.subplot(2,1,1)
# plt.plot(Lacc[:,0])
