...
def estIMU_HS_MS(acc,gyr,t):
    ...
    HS_sig = (np.gradient(acc[:,0],t))**2
    gyr_energy = (np.linalg.norm(gyr,axis=1))**2
    # Create a midstance detection signal
    idx = np.linalg.norm(acc,axis = 1) > 2*9.81 # Only want values above 2g as they will be excluded, may need to reduce this threshold
    MS_sig = gyr_energy
    MS_sig[idx] = 1e6
    ...
    while jj < len(HS_sig)-1000:
        if HS_sig[jj] > 1e9:
            # Find the maximum
            HS_peakidx = np.argmax(acc[jj-window:jj+window,0])
            HS.append(jj-window+HS_peakidx)
            jj = jj+window
        jj = jj+1
            
    # Find only the good strides
    stride_intervals = np.diff(HS)
    HS = np.array(HS[:-1])
    # Put limits on what is defined as ambulation: stride times between 0.5 sec and 1 sec
    GS = np.where((stride_intervals > 0.5/t[1])*(stride_intervals < 1.0/t[1]))[0]
    # Compute the mid-stance indicies
    MS = [(np.argmin(MS_sig[HS[jj]:HS[jj+1]])+HS[jj]) for jj in GS]
...
def computeRunSpeedIMU(acc,gyr,HS,MS,GS,t):
    ...
    for count,jj in enumerate(GS):
        ...
        theta = integrate.cumtrapz(gyr_stride_plus,time_stride_plus,initial=0,axis=0)
        ...
        for kk,ang in enumerate(theta):
            ...
            rotZ = np.array([[np.cos(ang[2]),-np.sin(ang[2]),0],
                            [np.sin(ang[2]),np.cos(ang[2]),0],
                            [0,0,1]])
            rotY = np.array([[np.cos(ang[1]),0,np.sin(ang[1])],
                            [0,1,0],
        ...
        IMU_vel = integrate.cumtrapz(acc_stride_plus_LCS,time_stride_plus,initial=0,axis=0)
...
fPath = 'C:\\Users\\eric.honert\\OneDrive - Boa Technology Inc\\Documents\Projects\\2022_TrialRun\\TestData\\IMU\\'
...
for ii in range(len(RLentries)):
    # Load the data
    RLdf = pd.read_csv(fPath + RLentries[ii],sep=',', header = 0)
    RHdf = pd.read_csv(fPath + RHentries[ii],sep=',', header = 0)
    
    LLdf = pd.read_csv(fPath + LLentries[ii],sep=',', header = 0)
    LHdf = pd.read_csv(fPath + LHentries[ii],sep=',', header = 0)
    
    # There will need to be a calibration file that will be used to assign 
    # the magnitude of gravity in each direction
    
    [Rtime,Racc,Rgyr] = align_fuse_extract_IMU(RLdf,RHdf)
    [Ltime,Lacc,Lgyr] = align_fuse_extract_IMU(LLdf,LHdf)
    
    # Note: Will need to write a cross-correlation function to align left and right IMUs based on the 3 syncing jumps
    
    # Identify the start of the trial that is indicated with the 3 hops
    # For right now: use known index of the start of running
    
    # Identify foot contacts
    Racc = Racc[R_start:,:]
    Rgyr = Rgyr[R_start:,:]
    ...
    # Identify foot contact, midstance, and good strides
    # [RHS,RMS,RGS] = estIMU_HS_MS(Racc,Rgyr,Rtime)
    [LHS,LMS,LGS] = estIMU_HS_MS(Lacc,Lgyr,Ltime)
    # Compute stride-by-stride running speed
...
plt.plot(Lacc[:,0])            
plt.plot(LHS[LGS],Lacc[LHS[LGS],0],'ro')
plt.plot(LMS,Lacc[LMS,0],'ko')    
