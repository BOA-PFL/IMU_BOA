...
@author: Eric.Honert
    ...
    Added inversion and eversion as unique outcome variables
...
# NumPy 2.0 removed np.trapz (renamed to np.trapezoid); alias for cross-version safety
...
def computeRunSpeedIMU(acc,gyr,HS,MS,GS,t):
    ...
    for count,jj in enumerate(GS):
        ...
        IMU_SL = trapz(IMU_vel[0:HS_idx,:], time_stride, axis=0)
...
# Obtain IMU signals
fPath = 'C:\\Users\\max.ferguson\\OneDrive - BOA Technology Inc\\Documents\\TrailAnkleMetrics\\InLab\\'
...
for ii in range(0,len(Lentries)):
    ...
    for jj in iGS:
        ...
        if Slope == 'n6':
           ...
           pIgyro.append(-np.max(igyr[iHS[jj]:appTO,2]))   # flip so inversion reads negative
...
for name, col in [('oSubject',oSubject),('oConfig',oConfig),('oSesh',oSesh),
                  ('oLabel',oLabel),('pJerk',pJerk),('pAcc',pAcc),('pGyr',pGyr),
                  ('rMLacc',rMLacc),('rIEgyro',rIEgyro),('pEgyro',pEgyro),
                  ('pIgyro',pIgyro),('oSpeed',oSpeed)]:
    print(f'{name:12s} {len(col)}')
    

outcomes = pd.DataFrame({'Subject':list(oSubject), 'Config': list(oConfig),'Sesh': list(oSesh), 'Label':list(oLabel), 'pJerk':list(pJerk), 'pAcc':list(pAcc), 'pGyr':list(pGyr),'rMLacc':list(rMLacc),'rIEgyro':list(rIEgyro),'pEgyro':list(pEgyro),'pIgyro': list(pIgyro),'imuSpeed':list(oSpeed)})

risky = outcomes[outcomes['pIgyro'] < -300]
print(len(risky), "strides below -300 deg/s")
print(risky.groupby('Config').size())

summary = (outcomes.assign(risky=outcomes['pIgyro'] < -300)
                   .groupby('Config')
                   .agg(strides=('risky','size'),
                        risky=('risky','sum'),
                        Mean=('pIgyro','mean'),
                        SD=('pIgyro','std')))
summary['pct'] = 100*summary['risky']/summary['strides']
print(summary)

n = (outcomes['pIgyro'] < -300).sum()
print(n, "strides below -300 deg/s")
outcomes[outcomes['pIgyro'] < -300].groupby(['Config','Label']).size()


if save_on == 1:
    outcomes.to_csv('C:\\Users\\max.ferguson\\OneDrive - BOA Technology Inc\\Documents\\TrailAnkleMetrics\\InLab\\InLabIMUmetrics_wInversionInLab.csv',header=True)
elif save_on == 2:
    outcomes.to_csv('C:\\Users\\max.ferguson\\OneDrive - BOA Technology Inc\\Documents\\TrailAnkleMetrics\\InLab\\InLabIMUmetrics_wInversionInLab.csv',mode = 'a',header=False)

import matplotlib.pyplot as plt

p = outcomes['pIgyro'].dropna()
fig, ax = plt.subplots(figsize=(9,5))
ax.hist(p, bins=80, color='tab:orange', edgecolor='white', linewidth=.3)
ax.axvline(-300, color='k', ls='--', lw=1)
ax.set_xlabel('pIgyro [deg/s]'); ax.set_ylabel('stride count')
ax.set_title(f'Inversion across all strides (n={len(p):,})')
plt.tight_layout(); plt.show()



summary.to_csv('C:\\Users\\max.ferguson\\OneDrive - BOA Technology Inc\\Documents\\TrailAnkleMetrics\\InLab\\condition_summaryInLab.csv', index=True)

# %% LMEM



#RiskModel = glmer(risky ~ outcomes.Config + (1|Subject) + offset(log(total_steps)), family=poisson/nbinom)
